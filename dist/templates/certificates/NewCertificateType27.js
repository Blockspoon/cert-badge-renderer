"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.createNewCertificateType27 = createNewCertificateType27;
function createNewCertificateType27({ mainColor = "#96A6B4", subColor = "#96A6B4", extraColor1 = "#96A6B4", extraColor2 = "#96A6B4", }) {
    const paintId0 = `paint0_linear_${Math.random().toString(36).substr(2, 9)}`;
    const paintId1 = `paint1_linear_${Math.random().toString(36).substr(2, 9)}`;
    const paintId2 = `paint2_linear_${Math.random().toString(36).substr(2, 9)}`;
    const paintId3 = `paint3_linear_${Math.random().toString(36).substr(2, 9)}`;
    return `
     <svg
        width="100%"
        height="100%"
        viewBox="0 0 1153 814"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <g clipPath="url(#clip0_1917_1284)">
          <rect
            width="1152"
            height="814"
            transform="translate(0.847656)"
            fill="white"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M84.9206 316.325H84.9318V290.949L-5.15227 290.949L-5.15227 316.325H-5.14164C-5.14876 316.655 -5.15234 316.985 -5.15234 317.317C-5.15234 342.193 15.0136 362.359 39.8895 362.359C64.7654 362.359 84.9313 342.193 84.9313 317.317C84.9313 316.985 84.9277 316.655 84.9206 316.325Z"
            fill="${subColor}"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M1080.33 540.393L1080.32 540.393L1080.32 562.701L1159.52 562.701L1159.52 540.393L1159.51 540.393C1159.51 540.103 1159.52 539.813 1159.52 539.521C1159.52 517.653 1141.79 499.926 1119.92 499.926C1098.05 499.926 1080.32 517.653 1080.32 539.521C1080.32 539.813 1080.33 540.103 1080.33 540.393Z"
            fill="${subColor}"
          />
          <rect
            x="84.8477"
            y="292"
            width="90"
            height="45"
            transform="rotate(-180 84.8477 292)"
            fill="#002333"
          />
          <rect x="1079.85" y="562" width="80" height="40" fill="#002333" />
          <path
            d="M39.8477 315L-5.15234 269.912L-5.15234 242L84.8477 242L84.8477 269.912L39.8477 315Z"
            fill="white"
          />
          <path
            d="M1119.85 542L1159.85 581.529L1159.85 606L1079.85 606L1079.85 581.529L1119.85 542Z"
            fill="white"
          />
          <path
            d="M45.3477 287L-0.15235 242L90.8477 242L45.3477 287Z"
            fill="#002333"
          />
          <path
            d="M1115.35 566L1154.85 606L1075.85 606L1115.35 566Z"
            fill="#002333"
          />
          <rect
            x="138.848"
            y="1"
            width="114"
            height="44"
            transform="rotate(90 138.848 1)"
            fill="${subColor}"
          />
          <rect
            x="1032.85"
            y="817"
            width="99"
            height="39"
            transform="rotate(-90 1032.85 817)"
            fill="${subColor}"
          />
          <rect
            x="90.8477"
            y="107"
            width="101"
            height="91"
            transform="rotate(90 90.8477 107)"
            fill="${subColor}"
          />
          <rect
            x="1075.85"
            y="724"
            width="88"
            height="79"
            transform="rotate(-90 1075.85 724)"
            fill="${subColor}"
          />
          <rect
            x="95.8477"
            y="1"
            width="114"
            height="59"
            transform="rotate(90 95.8477 1)"
            fill="#002333"
          />
          <rect
            x="1070.85"
            y="817"
            width="99"
            height="52"
            transform="rotate(-90 1070.85 817)"
            fill="#002333"
          />
          <path
            d="M94.8477 58L36.1712 115L-0.152341 115L-0.152336 0.999996L36.1712 0.999997L94.8477 58Z"
            fill="white"
          />
          <path
            d="M1071.85 767.5L1123.11 718L1154.85 718L1154.85 817L1123.11 817L1071.85 767.5Z"
            fill="white"
          />
          <path
            d="M58.8477 58L-0.152345 115L-0.15234 0.999997L58.8477 58Z"
            fill="${mainColor}"
          />
          <path
            d="M1103.85 767.5L1154.85 718L1154.85 817L1103.85 767.5Z"
            fill="${mainColor}"
          />
          <path
            d="M195.348 59L138.848 1L251.848 1.00001L195.348 59Z"
            fill="#002333"
          />
          <path
            d="M983.348 767L1032.85 817L933.848 817L983.348 767Z"
            fill="#002333"
          />
          <path
            d="M187.848 67L169.848 85L169.848 49L187.848 67Z"
            fill="#002333"
          />
          <path
            d="M989.848 760L1005.85 744L1005.85 776L989.848 760Z"
            fill="#002333"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M98.254 156.049C120.327 156.049 138.221 138.155 138.221 116.082C138.221 115.524 138.209 114.968 138.187 114.416L58.3212 114.416C58.2985 114.968 58.2871 115.524 58.2871 116.082C58.2871 138.155 76.1809 156.049 98.254 156.049Z"
            fill="${mainColor}"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M1068.61 681.289C1049.21 681.289 1033.48 697.019 1033.48 716.423C1033.48 716.914 1033.49 717.402 1033.51 717.888L1103.72 717.888C1103.74 717.402 1103.75 716.914 1103.75 716.423C1103.75 697.019 1088.02 681.289 1068.61 681.289Z"
            fill="${mainColor}"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M30.2959 600.07C30.2959 595.061 26.2349 591 21.2255 591C21.099 591 20.973 591.003 20.8477 591.008L20.8477 609.133C20.973 609.138 21.099 609.141 21.2255 609.141C26.2349 609.141 30.2959 605.08 30.2959 600.07Z"
            fill="${subColor}"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M90.6483 208.295L90.6484 208.295L1.19006 120.735L1.19005 208.295L90.6483 208.295Z"
            fill="white"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M90.6484 208.294L1.19109 309.81L1.19109 208.294L90.6484 208.294Z"
            fill="${mainColor}"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M19.2637 208.29C19.2637 208.29 19.2637 208.29 19.2637 208.29C19.2637 197.928 27.7595 189.527 38.2397 189.527C48.7199 189.527 57.2157 197.928 57.2157 208.29C57.2157 208.29 57.2157 208.29 57.2157 208.29L19.2637 208.29Z"
            fill="${mainColor}"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M57.2148 208.294C57.2148 208.294 57.2148 208.294 57.2148 208.294C57.2148 218.657 48.719 227.057 38.2388 227.057C27.7587 227.057 19.2628 218.657 19.2628 208.294C19.2628 208.294 19.2628 208.294 19.2628 208.294L57.2148 208.294Z"
            fill="white"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M1075.3 635.36L1075.3 635.36L1153.94 712.332L1153.94 635.36L1075.3 635.36Z"
            fill="white"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M1075.3 635.361L1153.94 546.121L1153.94 635.361L1075.3 635.361Z"
            fill="${mainColor}"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M1138.05 635.365C1138.05 635.365 1138.05 635.365 1138.05 635.365C1138.05 644.475 1130.58 651.859 1121.37 651.859C1112.16 651.859 1104.69 644.475 1104.69 635.365C1104.69 635.365 1104.69 635.365 1104.69 635.365L1138.05 635.365Z"
            fill="${mainColor}"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M1104.69 635.361C1104.69 635.361 1104.69 635.361 1104.69 635.361C1104.69 626.251 1112.16 618.867 1121.37 618.867C1130.58 618.867 1138.05 626.251 1138.05 635.361C1138.05 635.361 1138.05 635.361 1138.05 635.361L1104.69 635.361Z"
            fill="white"
          />
          <path
            d="M305.06 28.7687L261.293 28.3175L304.609 -14.9992L305.06 28.7687Z"
            fill="${mainColor}"
          />
          <path
            d="M952.504 727.629L990.98 728.025L952.901 766.104L952.504 727.629Z"
            fill="${mainColor}"
          />
          <path
            d="M156.848 115L279.848 -7"
            stroke="${mainColor}"
            strokeWidth="2"
          />
          <path
            d="M1017.85 718L908.848 825"
            stroke="${mainColor}"
            strokeWidth="2"
          />
          <path
            d="M12.0939 832.049L12.801 682.849"
            stroke="${mainColor}"
            strokeWidth="2"
          />
          <path
            d="M176.848 112L299.848 -11"
            stroke="${mainColor}"
            strokeWidth="2"
          />
          <path
            d="M986.848 731L878.848 839"
            stroke="${mainColor}"
            strokeWidth="2"
          />
          <path
            d="M23.4064 849.906V700"
            stroke="${mainColor}"
            strokeWidth="2"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M97.7413 642.333L78.8477 661.227L97.7413 680.12V642.333Z"
            fill="${mainColor}"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M97.7411 665.786L116.635 646.893L97.7411 627.999L97.7411 665.786Z"
            fill="#002333"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M219.848 395L227.848 387L219.848 379L219.848 395Z"
            fill="black"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M933.848 395.25L925.848 387.25L933.848 379.25L933.848 395.25Z"
            fill="black"
          />

          <rect
            x="18.8477"
            y="388"
            width="60"
            height="60"
            fill="url(#pattern0_1917_1284)"
          />

          <rect x="627.848" y="635" width="80" height="80" />

          <rect x="736.848" y="635" width="80" height="80" />

          <rect x="845.848" y="635" width="80" height="80" />

          <mask id="path-79-inside-1_1917_1284" fill="white"></mask>

          <mask
            id="mask0_1917_1284"
            maskUnits="userSpaceOnUse"
            x="952"
            y="167"
            width="122"
            height="59"
          ></mask>

          <rect
            x="999.514"
            y="70.3333"
            width="26.6667"
            height="26.6667"
            fill="url(#pattern1_1917_1284)"
          />
        </g>
        <defs>
          <pattern
            id="pattern0_1917_1284"
            patternContentUnits="objectBoundingBox"
            width="1"
            height="1"
          ></pattern>
          <pattern
            id="pattern1_1917_1284"
            patternContentUnits="objectBoundingBox"
            width="1"
            height="1"
          >
            <use transform="translate(-0.131198 -0.126971) scale(0.00353698)" />
          </pattern>
          <clipPath id="clip0_1917_1284">
            <rect
              width="1152"
              height="814"
              fill="white"
              transform="translate(0.847656)"
            />
          </clipPath>
          <image id="image0_1917_1284" width="357" height="357" />
          <image id="image1_1917_1284" width="360" height="360" />
        </defs>
      </svg>
  `;
}
